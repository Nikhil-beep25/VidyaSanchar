...
async function main() {
    ...
    const parentData = [
        {
            email: 'anil.gupta@parent.sms.edu.in',
            name: 'Anil Gupta',
    ...
    const studentData = [
        {
            email: 'rahul.gupta@student.sms.edu.in',
            name: 'Rahul Gupta',
            rollNumber: '10A01',
            ...
            bloodGroup: 'O+0',
            classId: classA.id,
            parentId: parents[0].id, // Anil Gupta
    ...
    for (const s of studentData) {
        const user = await prisma.user.create({
            data: {
                ...
                passwordHash: defaultPasswordHash,
                name: s.name,
                role: client_1.RoleType.STUDENT,
                phone: '+919999988888',
    ...
    for (const date of dates) {
        ...
        await prisma.attendance.create({
            data: {
                date,
                status: date.getDay() === 3 ? client_1.AttendanceStatus.ABSENT : client_1.AttendanceStatus.PRESENT,
    ...
    await prisma.payment.create({
        data: {
            ...
            status: client_1.PaymentStatus.PARTIAL,
    ...
    await prisma.bookIssue.create({
        data: {
            ...
            status: client_1.IssueStatus.ISSUED,
        }
    });
    await prisma.bookIssue.create({
        data: {
            ...
            status: client_1.IssueStatus.OVERDUE,
            fineAmount: 50.0,
        }
    });
